...
MAX_RETRIES=30
RETRY_COUNT=0
until docker compose exec -T spire-server /opt/spire/bin/spire-server healthcheck ${SOCKET_ARG} > /dev/null 2>&1 || [ $RETRY_COUNT -eq $MAX_RETRIES ]; do
...
if echo "$AGENT_LIST" | grep -q "spiffe://demo.local/node/agent"; then
    ...
    AGENT_RETRIES=0
    until docker compose exec -T spire-server /opt/spire/bin/spire-server agent list ${SOCKET_ARG} 2>/dev/null | grep -q "spiffe://demo.local/node/agent" || [ $AGENT_RETRIES -eq 25 ]; do
